...
simfreqs = [200 * i for i in range(1, threshsize+1)]
...
def simulation(t,x,y,simfreqs):
    base=random.random()*4096
    L=base
    R=base
    for i in range(len(x)):
        val=math.sin(t*simfreqs[i]/(2*math.pi))*4096
        L+=val/(math.sqrt(math.pow(x[i],2)+math.pow(y[i],2)))*4096
        R+=val/(math.sqrt(math.pow(x[i]-micDist,2)+math.pow(y[i],2)))*4096
    return L,R
...
#     for i in range(windowsize/2):
#         FREQSR[i]=np.sqrt(real[i]**2+imag[i]**2)
#     max=np.max(FREQSL)
#     for i in range(windowsize/2):
#         FREQSR[i]*=4096/max
#         FREQSL[i]*=4096/max
...
# #     FFRB= np.roll(FFRB,1)
#     FBRB= np.roll(FBRB,1)
#     FBLB[0],FBRB[0]=simulation(t/timerFreq,x,y,simfreqs)
#   
...
#     FFRB= np.roll(FFRB,1)
    FBRB= np.roll(FBRB,1)
    FBLB[0],FBRB[0]=simulation(t/timerFreq,x,y,simfreqs)
    FREQSL,a=np.fft.fft(FBLB)
#     for i in range(windowsize/2):
#         FREQSL[i]=math.sqrt(math.pow(real[i],2)+math.pow(imag[i],2))
    FREQSR,a=np.fft.fft(FBRB)
#     for i in range(windowsize/2):
#         FREQSR[i]=math.sqrt(real[i]**2+imag[i]**2)
    a=''
    max=np.max(FREQSL)
    for i in range(windowsize/2):
        FREQSR[i]*=4096/max
...
#     print(FREQSL)
    THRESHSL=np.sort(FREQSL)[windowsize//2-threshsize:windowsize//2]
    THRESHSR=np.sort(FREQSR)[windowsize//2-threshsize:windowsize//2]
    THRESHSHZ=np.argsort(FREQSL,axis=0)[windowsize//2-threshsize:windowsize//2]
    for i in range(len(THRESHSHZ)):
        THRESHSHZ[i]*=df
#     checkMemory()
